...
describe("llmAuth schema", () => {
	...
	it("rejects unknown type", () => {
		expect(() =>
			llmAuthSchema.parse({ type: "bedrock", token: "x" }),
		).toThrow();
	});
	...
	it("parses openai variant with token and model", () => {
		const auth = llmAuthSchema.parse({
			type: "openai",
			token: "sk-openai-xxx",
			model: "gpt-4o-mini",
		});
		expect(auth.type).toBe("openai");
		if (auth.type === "openai") {
			expect(auth.token).toBe("sk-openai-xxx");
			expect(auth.model).toBe("gpt-4o-mini");
		}
	});

	it("parses openrouter variant with token and model", () => {
		const auth = llmAuthSchema.parse({
			type: "openrouter",
			token: "sk-or-xxx",
			model: "openrouter/auto",
		});
		expect(auth.type).toBe("openrouter");
		if (auth.type === "openrouter") {
			expect(auth.token).toBe("sk-or-xxx");
			expect(auth.model).toBe("openrouter/auto");
		}
	});

	it("rejects openai missing token", () => {
		expect(() =>
			llmAuthSchema.parse({ type: "openai", model: "gpt-4o-mini" }),
		).toThrow();
	});

	it("rejects openai missing model", () => {
		expect(() =>
			llmAuthSchema.parse({ type: "openai", token: "sk-openai-xxx" }),
		).toThrow();
	});

	it("rejects openai empty token and empty model", () => {
		expect(() =>
			llmAuthSchema.parse({
				type: "openai",
				token: "",
	...
	it("rejects openrouter missing token", () => {
		expect(() =>
			llmAuthSchema.parse({ type: "openrouter", model: "openrouter/auto" }),
		).toThrow();
	});

	it("rejects openrouter missing model", () => {
		expect(() =>
			llmAuthSchema.parse({ type: "openrouter", token: "sk-or-xxx" }),
		).toThrow();
	});

	it("rejects openrouter empty token and empty model", () => {
		expect(() =>
			llmAuthSchema.parse({
				type: "openrouter",
				token: "",
	...
	it("accepts MCP servers and provenance trailers, defaulting both", () => {
		...
		expect(() =>
			taskSpecSchema.parse({
				...base,
				mcpServers: [
					{ name: "Bad Name!", type: "http", url: "https://x.dev" },
				],
			}),
		).toThrow();
	});
