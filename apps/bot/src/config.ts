...
const configSchema = z.object({
	...
	/** Fall back to CHAT_FALLBACK_MODEL when the primary chat model is rate-limited. */
	...
	/** Upper bound (seconds) on a single retry backoff delay for LLM calls. */
	RETRY_MAX_DELAY_SECONDS: z.coerce.number().int().min(0).max(30).default(5),
	/** Wall-clock timeout (seconds) for a mention-classifier LLM call. */
	...
	VERIFY_RESERVE_FRACTION: z.coerce.number().min(0).max(0.9).default(0.3),
