...
import { callWithRetry, type RetryPolicy } from "../llm/retry.js";
import {
...
export async function handleMention(
	...
	const guild = await ensureGuild(ctx.db, message.guildId, ctx.config);
	...
	try {
		...
		// Structured-result orchestration (Req 8): every classified failure is
		// rendered as exactly one mention-safe message via the message-builder —
		// no generic fallback string, no reply generation after a classify failure.
		const providerType = llm.auth.type;
		const customModelName =
			llm.auth.type === "custom" ? llm.auth.model : null;
		const policy: RetryPolicy = {
			maxRetryDelayMs: ctx.config.RETRY_MAX_DELAY_SECONDS * 1000,
		};
		const timeoutMs = ctx.config.CLASSIFIER_TIMEOUT_SECONDS * 1000;

		const classify = await callWithRetry(
			() =>
				classifyIntent(llm.auth, ctx.config.CHAT_MODEL, chatCtx, {
					timeoutMs,
				}),
			policy,
		);
		...
		if (decision.action === "reply") {
			let reply1 = await callWithRetry(
				() =>
					generateChatReply(llm.auth, ctx.config.DEFAULT_MODEL, chatCtx, {
						timeoutMs,
					}),
				policy,
			);

			// Rate-limited on the primary model: when fallback is enabled and the
			// Fallback_Model is distinct, retry once on it and prefix the lighter-
			// model notice on success; otherwise fall through to the rate-limit
			// message (Req 6.3–6.7).
			if (
				!reply1.ok &&
				reply1.failure.mode === "rate_limited" &&
				...
				reply1 = fb; // fall through to the rate-limit message (Req 6.6)
			}

			if (reply1.ok) {
				for (const chunk of chunkText(reply1.text, 1900)) {
					await reply(message, chunk);
				}
				return;
			}
		...
		// ask / code / propose_code → Task_Path (preflight handles its failures).
