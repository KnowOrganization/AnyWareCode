...
import { canInvoke, capState, ensureGuild, planHasFeature } from "./gates.js";
import {
	handleBillingButton,
...
async function startAgentTask(
	...
	if (interaction.channel?.isThread()) {
		await interaction.editReply({
			content:
				"Run this in a regular text channel; I'll open a thread there.",
		});
		return;
	}
	...
	const squadN =
		mode === "code" && !planNow
			? interaction.options.getInteger("squad")
			: null;
	if (squadN !== null && squadN !== undefined) {
		if (
			squadN > ctx.config.SQUAD_MAX ||
			!(await squadAllowed(ctx, guild.planId))
		) {
			await interaction.editReply({
				content:
					squadN > ctx.config.SQUAD_MAX
	...
	void launched.outcome.catch((err) =>
		captureError(err, {
			msg: "code task outcome rejected",
			channelId: interaction.channelId,
		}),
	);
}
...
async function handleCancel(
	...
	await interaction.reply({
		content: cancelled ? "🛑 Cancelling…" : "No running task in this thread.",
	...
	if (sub === "review") {
		...
		if (channel) {
			...
			if (!resolved?.isSendable()) {
				await interaction.reply({
					content:
						"I can't send messages in that channel — pick one where I have Send Messages.",
					flags: MessageFlags.Ephemeral,
				});
				return;
			}
		...
		await ctx.db
			...
			.onConflictDoUpdate({
				target: [
					schema.repoSettings.guildId,
					schema.repoSettings.repoFullName,
				],
				set: {
					autoReview: Boolean(channel),
					reviewChannelId: channel?.id ?? null,
...
async function handleConfigIssues(
	...
	if (!channel) {
		...
		await interaction.reply(
			`✅ Issue feed for \`${repoFullName}\` turned off.`,
		);
		return;
	}
...
async function handleButton(
	...
	if (action === "merge") {
		...
		void postShipLog(
			...
			captureError(err, { msg: "ship log (merge button) failed" }),
		);
		return;
	}
