...
export class TaskOrchestrator {
	...
	forwardThreadMessage(threadId: string, author: string, text: string): void {
		...
		if (model?.[1]) {
			...
			if (allowed)
				task.handle?.send({ type: "set_model", model: requested });
			else
				task.handle?.send({
					type: "user_message",
					author: "system",
	...
	private async execute(
		...
		const resolved = await resolveLlmAuth(
			this.db,
			this.config,
			params.guildId,
		);
		...
		// Captured so the discriminated-union type stays narrowed through the long
		// run loop below (property-access narrowing on resolved.auth is lost after
		// the intervening awaits/calls).
		...
		try {
			...
			await thread.send(`⚠️ Couldn't start the task container.${detail}`);
			return out("failed");
		}
		task.handle = handle;
		...
		try {
			...
			await thread
				.send(
					"⚠️ Couldn't post the progress message — the task was aborted.",
				)
				.catch(() => {});
			return out("failed");
		}
		...
		const updater = new ThrottledUpdater(
			async () => {
				await progressMessage.edit({
					embeds: [progressEmbed(renderer.render())],
				});
			},
			2000,
				...
				if (event.type === "assistant_text") {
					for (const chunk of chunkText(event.text, 2000)) {
						...
						await thread.send(chunk).catch((err) =>
							captureError(err, {
								msg: "assistant_text send failed",
								taskId,
							}),
						);
					}
		...
		if (errorMessage) {
			await this.settle(task, "failed");
			// OpenAI-compatible providers run through the runner's preflight +
			// translation sidecar. When that step fails the task could not run on
			// the configured provider at all, so post the clear, provider-named
			// failure that never names another provider/model and never retries
			// elsewhere (Req 7.3, 7.4). settle() above wrote status=failed only —
			// no diff/PR is persisted, so there is no partial result.
			if (
				(auth.type === "openai" || auth.type === "openrouter") &&
				isPreflightOrTranslatorFailure(errorMessage)
			) {
				await thread.send(buildProviderUnavailableMessage(auth.type));
			...
			embeds: [
				new EmbedBuilder()
					...
					.setTitle(
						verified
							? `🔀 PR #${prNumber} ready`
							: `🔀 PR #${prNumber} — checks failing, review`,
					)
					.setURL(prUrl)
