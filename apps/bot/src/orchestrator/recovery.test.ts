...
describe("recoverStaleTasks", () => {
  it("marks stale tasks failed, refunds usage, and notifies each thread", async () => {
    const stale = [
      { id: "t1", guildId: "g1", mode: "code" as const, threadId: "th1", status: "running", fundedBy: "plan" as const },
      { id: "t2", guildId: "g1", mode: "ask" as const, threadId: "th2", status: "queued", fundedBy: "plan" as const },
    ];
    ...
    // 1 update marks the batch failed; refundUsage issues 1 update per task.
