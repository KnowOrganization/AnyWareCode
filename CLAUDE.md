...
AnywhereCode is a Discord bot that runs a shared coding agent against a server's
...
docker build -f apps/runner/Dockerfile -t anywherecode-runner .   # runner only (built from REPO ROOT)
```
...
- **Git happens only in `runner/git.ts`.** The agent is prompted (and `allowedTools`
  ...
  ever pushes to the default branch. Branch namespace: `anywherecode/<taskId>`.
...
- **LLM credential resolution order:** guild row (encrypted) > platform `ANTHROPIC_API_KEY`
  > block. Undecryptable guild credential → surfaces explicit "reconnect" error, never
  silently falls back to platform key. `CREDENTIAL_SECRET` rotation bricks stored creds;
  document that rotation requires all guilds to `/connect llm` again.
